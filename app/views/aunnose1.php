<?php if (!$tieneGestionarUsuarios && !$tieneVerConsultas && !$tieneGestionarRolesPermisos): ?>
                ...
                <tbody>
                    <?php foreach ($misConsultas as $c): ?>
                        <tr class="tr-body-consultas">
                            ...
                            <td class="td-consultas"><?= !empty($c['sintomas']) ? e(implode(', ', $c['sintomas'])) : 'Ninguno' ?></td>
                            <td class="td-consultas">
                                <?php if (!empty($c['diagnosticos'])): foreach ($c['diagnosticos'] as $diag): ?>
                                    <div><strong><?= e($diag['codigo_icd_diagnostico']) ?></strong> - <?= e($diag['patologia'] ?? 'Sin detalle') ?></div>
                                <?php endforeach; else: ?> Sin diagnóstico <?php endif; ?>
                            </td>
    ...
    <?php endif; ?>
<?php else: ?>
    <!-- Vista Dashboard Administrativo / Médico -->
    <div class="dashboard-container">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
            ...
            <?php if ($tieneGenerarReportes): ?>
                <a name="openModal" data-modal="modalReporteMorbilidad" class="action-card__button btn-generar-reporte" href="#" style="background-color: #0284c7;">Generar Reporte de Morbilidad</a>
            <?php endif; ?>
        </div>
