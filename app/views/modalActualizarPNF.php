<div class="action-card">
    ...
    <p class="action-card__p">Modificación de Programa Nacional de Formación (PNF)</p>
    
    <form id="formActualizarPNF" class="action-card__form--registrar-usuarios">
        <div class="action-card__form--grid">
            <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?? '' ?>">
            
            <input type="hidden" name="form" value="actualizar_pnf">
            
            <input type="hidden" name="id_pnf" id="edit_id_pnf">

            <label for="edit_nombre_pnf" class="action-card__label">Nombre del PNF
                <input 
                    type="text" 
                    ...
                    placeholder="Ej. PNF en Informática"
                >
            </label>
        </div> 
        
        <div class="action-card__button-grid" style="margin-top: 15px;">
            <button type="button" class="action-card__button action-card__button--red" onclick="document.getElementById('modalActualizarPNF').close()">Cancelar</button>
            <button type="submit" class="action-card__button">Guardar Cambios</button>
        </div>
    </form>
</div>

<svg class="modal-crud__boton-cerrar" name="modalBotonCerrar" data-modal="modalActualizarPNF" onclick="document.getElementById('modalActualizarPNF').close()" fill="#000000" version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 460.775 460.775" xml:space="preserve">
