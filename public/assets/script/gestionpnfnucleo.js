document.addEventListener('DOMContentLoaded', function() {
    ...
    if (formRegistrarPNF) {
        formRegistrarPNF.addEventListener('submit', async function(e) {
            e.preventDefault() 
            ...
            try {
                const response = await fetch('api/pnfs', {
                    method: 'POST',
                    headers: {
                ...
                if (result.status === 'ok') {
                    ...
                    setTimeout(() => {
                        mostrarAlertaSeccion(result.message || "¡PNF registrado con éxito!", "success")
                    }, 50)
    ...
    if (formActualizarPNF) {
        formActualizarPNF.addEventListener('submit', async function(e) {
            e.preventDefault() 
            ...
            try {
                ...
                if (result.status === 'ok') {
                    ...
                    setTimeout(() => {
                        mostrarAlertaSeccion(result.message || "¡PNF actualizado con éxito!", "success");
                    }, 50);
    ...
    document.addEventListener('click', async function(e) {
        const eliminarPnf = e.target.closest('[name="eliminarPnf"]')
        const eliminarNucleo = e.target.closest('[name="eliminarNucleo"]')
        if (eliminarPnf) {
            e.preventDefault() 
            if (!confirm('¿Seguro que deseas eliminar este PNF?')) {
                return
            }
            const idPnf = eliminarPnf.dataset.id; 
            const modalEditar = document.getElementById('modalActualizarPNF')

            try {
                const response = await fetch(`api/pnfs/${idPnf}`, {
                    method: 'DELETE',
                })
                const result = await response.json().catch(() => null)
                ...
                if (result.status === 'ok') {
                    actualizarTablaCompleta()
                    mostrarAlertaSeccion(result.message || "¡PNF eliminado con éxito!", "success")
                } else {
                    mostrarAlertaSeccion(result.message || "Ocurrió un error al intentar eliminar.", "error")
                }
        ...
        if (eliminarNucleo) {
            ...
            const idNucleo = eliminarNucleo.dataset.id; 

            try {
                const response = await fetch(`api/nucleos/${idNucleo}`, {
                    method: 'DELETE',
                })
                const result = await response.json().catch(() => null)
...
// =========================================================================
const configurarEnvioFormularioNucleo = (idForm, idModal, msgExitoDefault, method) => {
...
configurarEnvioFormularioNucleo('formRegistrarNucleo', 'modalRegistrarNucleo', "¡Núcleo registrado con éxito!", "POST")
configurarEnvioFormularioNucleo('formActualizarNucleo', 'modalActualizarNucleo', "¡Núcleo actualizado con éxito!", "PUT")
